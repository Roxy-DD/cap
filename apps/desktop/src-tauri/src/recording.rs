...
pub enum InProgressRecording {
    Instant {
        ...
        video_upload_info: Option<VideoUploadInfo>,
                ...
                let actor_result: Result<InProgressRecording, anyhow::Error> = async {
                    match inputs.mode {
                        ...
                        RecordingMode::Instant => {
                            ...
                            // 离线版：仅当有上传信息时才创建上传任务
                            let progressive_upload = video_upload_info.clone().map(|info| {
                            ...
                            Ok(InProgressRecording::Instant {
                                handle,
                                progressive_upload,
                                video_upload_info: video_upload_info.clone(),
                                common: common.clone(),
    ...
    if let Some(recording) = recording_data {
        ...
        let video_id = match &recording {
            InProgressRecording::Instant {
                ...
                // 离线版：仅当有上传信息时才中止上传
                if let (Some(info), Some(upload)) = (video_upload_info, progressive_upload) {
                    debug!(
...
async fn handle_recording_finish(
    ...
    let (meta_inner, sharing) = match completed_recording {
        ...
        CompletedRecording::Instant {
            ...
            // 离线版：仅当有上传信息时才执行上传逻辑
            if let (Some(progressive_upload), Some(video_upload_info)) =
                (progressive_upload, video_upload_info.clone())
            {
                ...
                // 离线模式：无上传信息，仅保存本地
                info!("Instant recording completed in offline mode (no upload)");
